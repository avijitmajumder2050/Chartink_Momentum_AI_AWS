...
| Backend EC2 instance | `i-0f85d60f510f41fbf` (t3.micro), security group `sg-073a1f6cc534ed636` — the original (`i-035c10ae55c3f8b9c`) was user-terminated 2026-09-18 and this one relaunched from the launch template on 2026-09-20; the scheduler Lambda (`deploy/lambda/handler.py`'s `INSTANCE_ID`), CloudFront's EC2 origin, and the scheduler role's IAM policy (see below) all had to be updated to match — the IAM one was missed initially and only caught when a real StopInstances call from ondemand.html failed with `UnauthorizedOperation` |
| Scheduler IAM role | `quantile-scheduler-lambda-role`, inline policy `quantile-scheduler-access` — its `Ec2StartStop` statement's `Resource` is the single instance ARN above, by design (least privilege) — **this means every time the instance is recreated (a real terminate, not just stop/start), this policy's Resource must be updated too**, via `aws iam put-role-policy` with the new ARN, or Start/Stop will silently 403 until someone notices |
